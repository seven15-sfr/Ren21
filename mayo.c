                    ...
                    switch (sub) {
                        case 1: // BUSCAR
                            if(Ptr == NULL) {
                                ...
                                int f = 0;
                                while(Ptraux != NULL) {
                                    if(Ptraux->d == buscar) { f = 1; break; }
                                    Ptraux = Ptraux->Ptisig;
                                }
                                if(f) printf("El dato %d ya existe en la lista \n", buscar);
                        ...
                        case 3: // REEMPLAZAR
                            if(Ptr == NULL) {
                                ...
                                Ptraux = Ptr;
                                while(Ptraux != NULL) {
                                    if(Ptraux->d == remplazar) Ptraux->d = nuevoVal;
                                    Ptraux = Ptraux->Ptisig;
                                }
                        ...
                        case 4: // ORDENAR (Burbuja simple)
                            if(Ptr == NULL || Ptr->Ptisig == NULL) {
                                printf("Lista vacia o con un solo elemento.\n");
                            } else {
                                struct Dato *i, *j;
                                int aux;
                                for(i = Ptr; i != NULL; i = i->Ptisig) {
                                    for(j = i->Ptisig; j != NULL; j = j->Ptisig) {
                                        if(i->d > j->d) {
                                            aux = i->d;
                                            i->d = j->d;
                                            j->d = aux;
